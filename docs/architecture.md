...
Child processes started by `ProcessExecutor` omit secret-bearing environment variables (`*_API_KEY`, `*_TOKEN`, `*_SECRET`, plus `WFX_API_KEY`, `OPENAI_API_KEY`, and `OPENROUTER_API_KEY`) before applying any caller overlay. `ChildProcessEnvironment` is the policy surface for those defaults. The `powershell` tool restores named parent variables only when `inherit_environment` is set.
